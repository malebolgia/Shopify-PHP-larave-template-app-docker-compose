...
How to run docker the application using docker-compose.yml and ngrok

###

1. Create an APP
   ref: https://shopify.dev/docs/apps/getting-started/create
...
![Create your app](https://github.com/malebolgia/Shopify-PHP-larave-template-app-docker-compose/blob/main/_assets/image/create-php-app.jpg)

This will prompt you to login to your account via browser and get your Client ID

![Deploy app](https://github.com/malebolgia/Shopify-PHP-larave-template-app-docker-compose/blob/main/_assets/image/app-deploy-new.jpg)

2. Once you created your account the client ID will be available in your app, update the docker-compose.yml replace `SHOPIFY_API_KEY=<your clientid>` with your clientid.

3. Rename web/env.laravel as your .env

4. In your root above the web folder run

```bash
docker compose up -d
```

5. Since our docker using the volume web, so we don't need to rebuild all the time, run below after running the initial docker compose

```bash
 cd web
 docker run --rm -it -v ${pwd}:/app prooph/composer:8.1 install
```

6. Create db.sqlite under web/storage this will be the initial database

7. Check with `docker ps` if your container is running we can run additional requirement below, replace <containername> with your container

```bash
 docker exec <containername> php /app/artisan key:generate
 docker exec <containername> php /app/artisan migrate
```

8. Once your container is running in 8081 port, you can run now ngrok

```bash
 ngrok http 8081
```

![Ngrok](https://github.com/malebolgia/Shopify-PHP-larave-template-app-docker-compose/blob/main/_assets/image/ngrok-url-8081.jpg)

9. Open new terminal and run `docker compose down` get your ngrok URL update web/.env `HOST_NAME=<your url>` replace <your url>, don't include https:// or http://

10. Run `docker compose up -d` again

11. You need to have a store or a dev store to test the application, get your dev store URL and run this to your browser
    ![Your Store list](https://github.com/malebolgia/Shopify-PHP-larave-template-app-docker-compose/blob/main/_assets/image/devstores.jpg)

```
https://<ngrok url>auth?shop=<yourdevstore>.myshopify.com
```

12. It might show an error like:
    `Oauth error invalid_request: The redirect_uri is not whitelisted`, to fix this just add your ngrok URL under `APP Setup`
    ![White list](https://github.com/malebolgia/Shopify-PHP-larave-template-app-docker-compose/blob/main/_assets/image/setup-app-whitelist.jpg)

13. Try again accessing `https://<ngrok url>auth?shop=<yourdevstore>.myshopify.com` It should now ask a permission for your dev store for you to accept.
    ![Store Permission](https://github.com/malebolgia/Shopify-PHP-larave-template-app-docker-compose/blob/main/_assets/image/connect-to-your-store.jpg)

Thats it you should be able to test your app with your app with your shopify :sparkles:

Happy Coding! :computer:

## Below instructions is from official Shopify you can use for your addional guide.
